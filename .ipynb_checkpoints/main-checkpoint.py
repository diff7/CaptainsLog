...
def get_words_count_dict(text_list, min_size=2, max_w=500):
    text_list.fillna("-1", inplace=True)
    text_list = text_list.astype(str)
    lists_dict = text_list.to_dict(orient="list")
    ...
    vectorizer = TfidfVectorizer(max_df=0.99, min_df=0.01, stop_words = 'english', max_features=max_w)
